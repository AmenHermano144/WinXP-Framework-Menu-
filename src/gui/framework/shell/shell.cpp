...
namespace ksd
{
    ...
    static void DrawTitleGlyph(const ImVec2& c, int glyph, ImU32 col)
    {
        if (glyph == 0)
        {
            s_dl->AddRectFilled(ImVec2(c.x - 4.f, c.y + 3.f), ImVec2(c.x + 4.f, c.y + 5.f), col);
        }
        else if (glyph == 1)
        {
            s_dl->AddRect(ImVec2(c.x - 5.f, c.y - 5.f), ImVec2(c.x + 5.f, c.y + 5.f), col, 0.f, 0, 1.6f);
            s_dl->AddRectFilled(ImVec2(c.x - 5.f, c.y - 5.f), ImVec2(c.x + 5.f, c.y - 3.f), col);
        }
        else
        {
            for (float t = -4.f; t <= 4.f; t += 1.f)
            {
    ...
    {
        const bool isClose = (glyph == 2);
        const bool hovered = enabled && ImGui::IsMouseHoveringRect(a, b);
        const bool held = hovered && ImGui::IsMouseDown(ImGuiMouseButton_Left);

        if (isClose)
        {
            style::CloseBtn(s_dl, a, b, held);
        }
        else
        {
            style::TitleBtnBlue(s_dl, a, b, held);
        }

        const float push = held ? 1.f : 0.f;
        const ImVec2 c((a.x + b.x) * 0.5f + push, (a.y + b.y) * 0.5f + push);

        const ImU32 glyphCol = isClose
            ? IM_COL32(255, 255, 255, 255)
            : (enabled ? IM_COL32(255, 255, 255, 255) : IM_COL32(255, 255, 255, 110));

        // subtle dark drop shadow behind the glyph for XP-style pop
        DrawTitleGlyph(ImVec2(c.x + 1.f, c.y + 1.f), glyph, IM_COL32(0, 30, 90, 130));
        DrawTitleGlyph(c, glyph, glyphCol);
    ...
    void BeginMain(const char* title)
    {
        ...
        style::FillFace(s_dl, s_winMin, s_winMax);
        style::FrameRing(s_dl, s_winMin, s_winMax);
        ...
        style::TitleBar(s_dl, titleMin, titleMax);

        ImFont* titleFont = fonts::Title();
        const float titleFontH = fonts::TitleSize();
        const float textY = titleMin.y + (style::TitleBarH - titleFontH) * 0.5f;
        const ImVec2 textPos(titleMin.x + 8.f, textY);
        s_dl->AddText(titleFont, titleFontH, ImVec2(textPos.x + 1.f, textPos.y + 1.f),
            IM_COL32(0, 30, 90, 140), title);
        s_dl->AddText(titleFont, titleFontH, textPos, style::TitleText, title);

        const float btnY = titleMin.y + (style::TitleBarH - style::TitleBtnH) * 0.5f;
        const float closeX = titleMax.x - style::TitleBtnW - 2.f;
        const float maxX = closeX - style::TitleBtnW - 2.f;
        const float minX = maxX - style::TitleBtnW;
        ...
        style::FillFace(s_dl, bodyMin, bodyMax);
        style::Etched(s_dl, bodyMin, bodyMax);
        s_bodyMin = bodyMin;
        s_bodyMax = bodyMax;
    ...
    bool TabButton(const char* label, int index)
    {
        ...
        if (active)
        {
            s_dl->AddRectFilledMultiColor(a, b,
        ...
        {
            s_dl->AddRectFilledMultiColor(a, b,
                IM_COL32(238, 244, 251, 255), IM_COL32(238, 244, 251, 255),
                IM_COL32(200, 218, 240, 255), IM_COL32(200, 218, 240, 255));
        }

        s_dl->AddLine(ImVec2(a.x, a.y), ImVec2(b.x, a.y), style::InputBorder, 1.f);
        s_dl->AddLine(ImVec2(a.x, a.y), ImVec2(a.x, b.y), style::InputBorder, 1.f);
        s_dl->AddLine(ImVec2(b.x - 1.f, a.y), ImVec2(b.x - 1.f, b.y), style::InputBorder, 1.f);
        ...
        s_dl->AddText(tp, active ? style::Text : IM_COL32(30, 60, 130, 255), label);
    ...
    void EndMain()
    {
        ...
        s_dl->AddText(ImVec2(statusMin.x + 5.f, ty), style::Text, "Win XP");
        const char* uid = "144777";
