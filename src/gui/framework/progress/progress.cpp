...
namespace ksd
{
    void ProgressBar(float fraction)
    {
        ...
        dl->AddRectFilled(a, b, style::ProgressBg);
        style::Sunken(dl, a, b);

        const ImU32 blockTop = IM_COL32(56, 195, 45, 255);
        const ImU32 blockBot = style::ProgressGreen;
        const float inset = 3.f;
        const float blockW = 8.f;
        const float gap = 2.f;
        ...
        {
            dl->AddRectFilledMultiColor(ImVec2(x, top), ImVec2(x + blockW, bot),
                blockTop, blockTop, blockBot, blockBot);
            x += blockW + gap;
        }
